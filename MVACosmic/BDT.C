...
void BDT( const TString& fin = "TMVA.root" )
{
   ...
   for (UInt_t im=0; im<path.size(); im++) {
      ...
      fname += wfile[im];
      // TString macro = Form( ".x BDT.C(0,\"%s\",\"%s\")", fname.Data(), methname[im].Data() );
      TString macro = Form( "BDTDraw(0,\"%s\",\"%s\")", fname.Data(), methname[im].Data() );
...
void BDTDraw( Int_t itree, TString wfile = "weights/TMVAnalysis_test_BDT.weights.txt", TString methName = "BDT", Bool_t useTMVAStyle = kTRUE ) 
{
